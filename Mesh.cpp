...
void bvh_delete(bvhnode *node) {
  ...
  else {  bvh_delete(node->data.children[0]); bvh_delete(node->data.children[1]);
    delete node;
  }
}
...
void Mesh::storeVBO() {
  ...
  for(long mtl_idx = 0; mtl_idx < (long)materials.size(); mtl_idx++) {
    ...
    mat_N_faces[mtl_idx] = mtl_faces.size();
    mat_offset[mtl_idx] = tri_vert.size();
...
// Build a BVH based on a buffer of triangles.
void Mesh::storeBVH(bool use_SAH) {
  if(triangles.size() == 0) storeTri();
  bvhroot = build_bvh(use_SAH, triangles, 0, triangles.size());
}
...
bool Mesh::check_intersect(bool useBVH, long &aabb_cnt, long &tri_cnt, int &mtl_idx, QVector3D &pos, QVector3D &norm, Ray &ray) {
  ...
  if(bvhroot == NULL || useBVH == false) {
	...
	else { best = isect; found_isect = true; }
      }
    }
  }
  else {
